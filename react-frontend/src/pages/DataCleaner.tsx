...
const CLEANING_OPTIONS = [
  ...
  { label: '15. Chuyển Jasper ➔ Oracle (DataGrip) ($P{var} ➔ :var)', value: 'jasperToOracle', exclusive: true },
  { label: '16. Chuyển Oracle ➔ Jasper (:var ➔ $P{var})', value: 'oracleToJasper', exclusive: true }
];

const DataCleaner: React.FC = () => {
  const [inputData, setInputData] = useState('');
  const [result, setResult] = useState('');
  const [selectedOptions, setSelectedOptions] = useState<string[]>(['mergeEmptyLines', 'removeJavaJsComments']);

  const handleOptionsChange = (checkedValues: string[]) => {
    const exclusiveValues = CLEANING_OPTIONS.filter((opt) => opt.exclusive).map((opt) => opt.value);
    const newlySelectedExclusive = checkedValues.find(
      (val) => exclusiveValues.includes(val) && !selectedOptions.includes(val)
    );

    if (newlySelectedExclusive) {
      setSelectedOptions([newlySelectedExclusive]);
    } else {
      const nonExclusiveOptions = checkedValues.filter((val) => !exclusiveValues.includes(val));
      if (nonExclusiveOptions.length > 0) {
        setSelectedOptions(nonExclusiveOptions);
      } else {
  ...
  const handleClean = () => {
    ...
    try {
      ...
      if (selectedOptions.includes('jasperToOracle')) {
        processed = processed.replace(/\$P\{([a-zA-Z_]\w*)\}/g, ':$1');
      }

      if (selectedOptions.includes('oracleToJasper')) {
        processed = processed.replace(/(?<!:):(?!:|=|\d)([a-zA-Z_]\w*)/g, '$$P{$1}');
      }
    ...
    >
      <SectionCard>
        ...
        <Checkbox.Group
          ...
          onChange={(checkedValues) => handleOptionsChange(checkedValues as string[])}
        >
          <Row gutter={[16, 12]}>
