...
public class ExcelController {
    ...
    @Operation(summary = "Cập nhật công thức cột Excel từ Mã nguồn sang Mã đích")
    @PostMapping(value = "/update-formula", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<?> updateExcelFormula(
            @Parameter(description = "File Excel cần update", content = @Content(mediaType = MediaType.MULTIPART_FORM_DATA_VALUE)) @RequestPart("file") MultipartFile file,
            @Parameter(description = "Tên cột mã nguồn hiện tại") @RequestParam("sourceCol") String sourceCol,
            @Parameter(description = "Tên cột mã đích cần đổi sang") @RequestParam("targetCol") String targetCol,
            @Parameter(description = "Tên cột chứa công thức") @RequestParam("formulaCol") String formulaCol) {

        if (file.isEmpty() || file.getOriginalFilename() == null || 
            (!file.getOriginalFilename().endsWith(".xlsx") && !file.getOriginalFilename().endsWith(".xls"))) {
            return ResponseEntity.badRequest().body("File không hợp lệ. Vui lòng upload file Excel (.xlsx hoặc .xls)");
        }

        try {
            byte[] excelBytes = excelParserService.updateFormulaInExcel(file, sourceCol, targetCol, formulaCol);
            String outputFilename = file.getOriginalFilename().replaceFirst("(?i)\\.xlsx?$", "_updated.xlsx");
            return ResponseEntity.ok()
